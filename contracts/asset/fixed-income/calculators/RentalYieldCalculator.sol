...
/// @title RentalYieldCalculator
/// @notice Calculates rental yield returns for Sukuk Ijarah structures.
/// @dev rateBps represents the annual rental rate.
contract RentalYieldCalculator is IReturnCalculator {
    ...
    function calculate(CalculationParams calldata params) external pure override returns (uint256 amount) {
        ...
        uint256 assetValue = abi.decode(params.extraData, (uint256));
        amount = (assetValue * params.rateBps * dayCount) / (yearBasis * BPS_DIVISOR);
    }
