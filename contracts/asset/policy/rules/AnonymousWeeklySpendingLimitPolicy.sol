...
import {PolicySpendingLimitExceeded} from "../../../library/Errors.sol";
contract AnonymousWeeklySpendingLimitPolicy is IPolicy, CBAccessControl {
    ...
    function check(address from, address to, uint256 amount) external override returns (address from_, address to_, uint256 amount_) {
        ...
        uint256 newSpent = limitOf[from].spent + amount;
        if (newSpent > weeklySpendingLimit) revert PolicySpendingLimitExceeded(from, newSpent, weeklySpendingLimit);
        limitOf[from].spent = newSpent;
