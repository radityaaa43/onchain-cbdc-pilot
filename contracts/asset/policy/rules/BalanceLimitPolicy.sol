...
import {BalanceLimitExceeded} from "../../../library/Errors.sol";
...
contract BalanceLimitPolicy is IPolicy, CBAccessControl {
     ...
     */
    uint256 private defaultLimit = 5_000_000;
    ...
    function setExempt(address account, bool exempt) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_isExempt[account] == exempt) revert AccountStatusUnchanged(account);
    ...
    function check(address from, address to, uint256 amount) external override returns (address from_, address to_, uint256 amount_) {
        if (_isExempt[from] || _isExempt[to]) return IPolicy._nextPolicy(from, to, amount);
        ...
        if (postBalance > limit) revert BalanceLimitExceeded(to, postBalance, limit);
    ...
    error AccountStatusUnchanged(address account);
}
