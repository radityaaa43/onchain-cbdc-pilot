...
export const bondMetadata = {
  ...
  setDltPlatform: (data: Record<string, unknown>) => dltTx<{ ok: boolean }>(`/bond-metadata/dlt-platform`, { data }),
  setCreditEvents: (data: Record<string, unknown>) => dltTx<{ ok: boolean }>(`/bond-metadata/credit-events`, { data }),
