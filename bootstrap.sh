...
# Read org domain and Besu NodePort for final output / block-wait
...
# Inject PG password into FireFly manifests (password generated by deploy-data.sh)
if [[ -f ".env.local" ]]; then
  _PG_PASS="$(grep "^POSTGRES_PASSWORD=" .env.local | tail -1 | cut -d= -f2)"
  if [[ -n "$_PG_PASS" ]]; then
    sed -i "s|PLACEHOLDER_PG_PASSWORD|${_PG_PASS}|g" \
      middleware/firefly/firefly-config.yaml \
      middleware/firefly/multiparty-app.yaml
    echo "Patched PostgreSQL password into FireFly manifests"
  fi
fi
